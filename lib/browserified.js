  ...
  loadDcCon = function() {
    ...
    dcConScript.onload = function() {
      if (dcConsData.length == 0) { debugLog("디씨콘을 불러오는 데 실패했습니다."); }
        ...
        // 태그로 랜덤 아이템 가져오기
        function getRandomItemByTag(tag) {
        ...
        applyDcCon = function(message, data) {
          ...
          // 태그 랜덤 처리 추가 (#태그명 형식)
          var tagPattern = /#([가-힣a-zA-Z0-9_]+)/g;
          var tagMatch;
          while ((tagMatch = tagPattern.exec(message)) !== null) {
            var tagName = tagMatch[1];
            var randomItem = getRandomItemByTag(tagName);
            
            if (randomItem) {
              var fileName = Array.isArray(randomItem.name) 
                ? randomItem.name[Math.floor(Math.random() * randomItem.name.length)]
                : randomItem.name;
              
              let src = configData.dcConsURI + dcConsSubURI + fileName + '?n=' + chatNumber;
              message = message.replace('#' + tagName, '<img class="dccon" src="' + src + '" />');
              
              // 사운드 재생
              if (randomItem.soundURL) {
                var soundFile = Array.isArray(randomItem.soundURL)
                  ? randomItem.soundURL[Math.floor(Math.random() * randomItem.soundURL.length)]
                  : randomItem.soundURL;
                
                let audio = new Audio(configData.dcConsURI + dcConsSubURI + soundFile);
                audio.autoplay = 1;
                audio.volume = 0.3;
                audio.load();
              }
